...
photos.forEach(([file,alt,caption],i)=>{const b=document.createElement('button');b.className='memory';b.setAttribute('aria-label','Perbesar foto '+(i+1)+': '+alt);b.innerHTML=`<img src="./${file}.webp" alt="${alt}" width="900" height="1200" loading="lazy"><span class="caption">${caption}</span><span class="number">${String(i+1).padStart(2,'0')} / 12</span>`;b.addEventListener('click',()=>{document.querySelector('#full-photo').src=`./${file}.webp`;document.querySelector('#full-photo').alt=alt;document.querySelector('#full-caption').textContent=caption;dialog.showModal()});gallery.appendChild(b)});
